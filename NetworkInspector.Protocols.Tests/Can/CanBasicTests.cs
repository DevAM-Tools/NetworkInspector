...
{
    private static byte[] _BuildClassic(uint canId, ReadOnlySpan<byte> data, bool extended = false, bool rtr = false)
    {
    ...
    private static byte[] _BuildFd(uint canId, ReadOnlySpan<byte> data, bool brs = false, bool esi = false)
    {
    ...
    {
        byte[] frame = _BuildClassic(0x123u, [0xDE, 0xAD, 0xBE, 0xEF]);
    ...
    {
        byte[] frame = _BuildClassic(0x1ABCDEFu, [], extended: true);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
    ...
    {
        byte[] frame = _BuildClassic(0x100u, [], rtr: true);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
    ...
    {
        byte[] frame = _BuildFd(0x456u, new byte[16], brs: true, esi: true);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
    ...
    {
        byte[] frame = _BuildClassic(0x123u, [0xDE, 0xAD]);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
    ...
    {
        byte[] frame = _BuildFd(0x456u, new byte[8], brs: true, esi: true);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
    ...
    {
        byte[] frame = _BuildFd(0x100u, new byte[8]);
        (Stack stack, Packet packet) = ProtocolTestHelper.BuildAndParse(frame, LinkType.CanSocketcan);
        using (stack)
        {
