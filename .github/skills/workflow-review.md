...
- If `reviews/brief_<slug>*.md` exists for this scope, read it first. Follow card order. Open each file link. Treat **How it serves** as claims to verify in the file. A slogan, ID-only, or copied **How it serves** is not a reason — read the file anyway.
