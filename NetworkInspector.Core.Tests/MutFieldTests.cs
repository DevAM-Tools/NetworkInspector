...
internal sealed class MutFieldTests
{
    ...
    public async Task MutField_TryGetFieldMutAt_ReturnsMutableHandle()
    {
        ...
        using (stack)
        {
            ...
            FieldId rootFieldId = root.FieldId;
            bool missing = packet.TryGetFieldMutAt(999, out _);
    ...
    public async Task MutField_AsField_AndIteration_Roundtrip()
    {
        ...
        using (stack)
        {
            ...
            int childCount = 0;
            foreach (Field _ in rootMut.Children())
            {
                childCount++;
            }

            int descendantCount = 0;
            foreach (Field _ in rootMut.Descendants())
            {
                descendantCount++;
            }
            ...
            await Assert.That(childCount).IsGreaterThan(0);
            await Assert.That(descendantCount).IsGreaterThan(childCount);
        }
    }
    ...
    public async Task MutField_TryCallNextProtocol_AllTables_ExerciseFastPaths()
    {
        ...
        using (stack)
        {
            ...
            await Assert.That(parent.NullTableHits).IsEqualTo(5);
            ...
            await Assert.That(packet.FieldCount()).IsGreaterThan(1);
        }
    }
        ...
        ProtocolTableId emptyAnyTable) : IProtocol
    {
        ...
        public ParseResult Parse(in MutField parentField, ReadOnlyMemory<byte> data, in ParseContext context)
        {
            ...
            if (parentField.TryCallNextProtocolU64(ProtocolTableId.Invalid, 7, payload, in context).TryGetValue(out _))
            {
                NullTableHits++;
            }
            if (parentField.TryCallNextProtocolU64(u64Table, 99, payload, in context).TryGetValue(out _))
            {
                EmptyMatchHits++;
            }
            if (parentField.TryCallNextProtocolU64(u64Table, 7, payload, in context).TryGetValue(out _))
            {
                SingleMatchHits++;
            }

            if (parentField.TryCallNextProtocolString(ProtocolTableId.Invalid, "hit", payload, in context).TryGetValue(out _))
            {
                NullTableHits++;
            }
            if (parentField.TryCallNextProtocolString(strTable, "miss", payload, in context).TryGetValue(out _))
            {
                EmptyMatchHits++;
            }
            if (parentField.TryCallNextProtocolString(strTable, "hit", payload, in context).TryGetValue(out _))
            {
                SingleMatchHits++;
            }

            if (parentField.TryCallNextProtocolBytes(ProtocolTableId.Invalid, new BytesKey([0xAB]), payload, in context).TryGetValue(out _))
            {
                NullTableHits++;
            }
            if (parentField.TryCallNextProtocolBytes(bytesTable, new BytesKey([0x00]), payload, in context).TryGetValue(out _))
            {
                EmptyMatchHits++;
            }
            if (parentField.TryCallNextProtocolBytes(bytesTable, new BytesKey([0xAB]), payload, in context).TryGetValue(out _))
            {
                SingleMatchHits++;
            }

            if (parentField.TryCallNextProtocolBool(ProtocolTableId.Invalid, true, payload, in context).TryGetValue(out _))
            {
                NullTableHits++;
            }
            if (parentField.TryCallNextProtocolBool(boolTable, false, payload, in context).TryGetValue(out _))
            {
                EmptyMatchHits++;
            }
            if (parentField.TryCallNextProtocolBool(boolTable, true, payload, in context).TryGetValue(out _))
            {
                SingleMatchHits++;
            }

            if (parentField.TryCallNextProtocolAny(ProtocolTableId.Invalid, payload, in context).TryGetValue(out _))
            {
                NullTableHits++;
            }

            if (parentField.TryCallNextProtocolAny(emptyAnyTable, payload, in context).TryGetValue(out _))
            {
                EmptyMatchHits++;
            }
            if (parentField.TryCallNextProtocolAny(anyTable, payload, in context).TryGetValue(out _))
            {
                SingleMatchHits++;
            }
    ...
    private sealed class MultiDispatchProtocol(ProtocolTableId tableId) : IProtocol
    {
        ...
        public ParseResult Parse(in MutField parentField, ReadOnlyMemory<byte> data, in ParseContext context)
        {
            if (parentField.TryCallNextProtocolU64(tableId, 9, data, in context).TryGetValue(out _))
            {
                MultiMatchHits++;
            }
            return 1;
    ...
    private sealed class HeuristicExerciseProtocol : IProtocol
    {
        ...
        public ParseResult Parse(in MutField parentField, ReadOnlyMemory<byte> data, in ParseContext context)
        {
            if (parentField.TryCallHeuristicProtocol(HeuristicProtocolTableId.Invalid, data, in context).TryGetValue(out _))
            {
                NullTableHit = true;
            }

            if (parentField.TryCallHeuristicProtocol(_TableId, data[..0], in context).TryGetValue(out _))
            {
                NoMatchHit = true;
            }

            if (parentField.TryCallHeuristicProtocol(_TableId, data, in context).TryGetValue(out _))
            {
                MatchHit = true;
            }
        ...
        ProtocolTableId anyTable) : IProtocol
    {
        ...
        public ParseResult Parse(in MutField parentField, ReadOnlyMemory<byte> data, in ParseContext context)
        {
            ...
            if (parentField.TryCallNextProtocolString(strTable, "x", payload, in context).TryGetValue(out _))
            {
                MultiMatchHits++;
            }
            if (parentField.TryCallNextProtocolBytes(bytesTable, new BytesKey([0x01]), payload, in context).TryGetValue(out _))
            {
                MultiMatchHits++;
            }
            if (parentField.TryCallNextProtocolBool(boolTable, true, payload, in context).TryGetValue(out _))
