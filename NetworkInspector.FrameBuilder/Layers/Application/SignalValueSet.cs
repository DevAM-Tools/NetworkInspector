...
/// <summary>
/// Mutable, per-signal value collection that pairs a <see cref="SignalPduLayout"/>
...
///   the <see cref="SignalPduLayer"/> applies the inverse linear scaling
...
/// </para>
/// <para>
/// Thread safety: not thread-safe. A <see cref="SignalValueSet"/> is built
/// per emitted frame and consumed once.
/// </para>
/// </remarks>
public sealed class SignalValueSet
{
    ...
    /// </summary>
    public static SignalValueSet For(SignalPduLayout layout) => new(layout);

    private SignalValueSet(SignalPduLayout layout)
    {
        _Layout = layout;
    }

    /// <summary>The layout this value set was built for.</summary>
    public SignalPduLayout Layout => _Layout;
    ...
    public SignalValueSet Set(string signalName, double value)
    {
    ...
    /// </summary>
    /// <returns>The same instance for fluent chaining.</returns>
    public SignalValueSet SetRaw(string signalName, ulong raw)
    {
        _Raw[signalName] = raw;
