...
/// <summary>
/// Signal Message application layer for the new <see cref="FrameStack"/> API.
...
/// in-memory <see cref="SignalMessageLayout"/> drives both sides via the test
...
/// </remarks>
public readonly struct SignalMessageLayer : IStatelessLayer, IPayloadLayer, IPseudoHeaderIndependent
{
    private readonly SignalMessageLayout? _Layout;
    private readonly SignalMessageValueSet? _Values;
    private readonly ReadOnlyMemory<byte> _RawBytes;

    /// <summary>
    /// Creates a Signal Message layer that renders the given <paramref name="layout"/>
    ...
    public SignalMessageLayer(SignalMessageLayout layout, SignalMessageValueSet values)
    {
        ArgumentNullException.ThrowIfNull(layout);
        ArgumentNullException.ThrowIfNull(values);
        if (!ReferenceEquals(values.Layout, layout))
        {
            throw new ArgumentException("SignalMessageValueSet was built for a different SignalMessageLayout instance.", nameof(values));
        }
        _Layout = layout;
        _Values = values;
        _RawBytes = default;
    ...
    private SignalMessageLayer(ReadOnlyMemory<byte> rawBytes)
    {
        _Layout = null;
        _Values = null;
        _RawBytes = rawBytes;
    ...
    /// <summary>
    /// Builds a Signal Message layer that emits <paramref name="bytes"/> verbatim
    /// without any bitfield encoding, for cases where the wire image is computed
    /// externally.  Prefer the
    /// <see cref="SignalMessageLayer(SignalMessageLayout, SignalMessageValueSet)"/> constructor
    /// for production code.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static SignalMessageLayer FromRawBytes(ReadOnlyMemory<byte> bytes) => new(bytes);
    ...
    public void WriteHeader(scoped Span<byte> dst)
    {
        ...
        SignalMessageLayout layout = _Layout;
        SignalMessageValueSet values = _Values!;
    ...
    /// raw value via <see cref="SignalMessageValueSet.SetRaw"/>, falls back to a
    /// physical value via <see cref="SignalMessageValueSet.Set"/> (no scaling for
    /// the selector itself), and otherwise throws so a missing mux value
    /// fails loudly instead of silently rendering group 0.
    /// </summary>
    private static ulong _ResolveMuxRaw(MuxSpec mux, SignalMessageValueSet values)
    {
        ...
        throw new InvalidOperationException(
            $"Multiplexer selector '{mux.Name}' has no value in the SignalMessageValueSet.");
    }
    ...
    /// </summary>
    private static void _EncodeSignal(scoped Span<byte> dst, SignalSpec sig, SignalMessageValueSet values)
    {
        ulong raw;
    ...
    /// <summary>
    /// Converts a physical (post-scaled) signal value into unsigned raw bits:
    /// <c>raw = round((physical - offset) / factor)</c>, then masked to
