...
public sealed partial class ProtocolGenerator
{
    ...
    private static string GenerateSource(ProtocolInfo info)
    {
        ...
        sb.AppendLine("// Copyright (c) DevAM and Network Inspector Contributors");
        ...
        sb.AppendLine($"    /// <summary>Implementation detail: the protocol's unique ID assigned during <see cref=\"RegisterFields\"/>. Available for handwritten partial methods such as <c>RecordProtocolPresence</c>.</summary>");
        ...
        sb.AppendLine("    /// <summary>Called at the end of RegisterFields, after all fields, tables, settings, and dispatch entries are registered and setting values are loaded into backing fields. Use this hook for config-driven setup (loading config files, building lookup dictionaries, registering additional dispatch entries via WhenProtocolTableRegistered, resolving cross-protocol field IDs via WhenFieldRegistered). Anything that does not require the frozen Stack belongs here, not in OnStartCustom.</summary>");
        sb.AppendLine($"    partial void RegisterFieldsCustom({_GloIStackBuilder} builder, {_GloProtocolId} protocolId);");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Called after the stack is built and frozen. Use this hook only for setup that requires the frozen Stack (e.g., pre-bound ParseDelegate caches obtained via stack.ResolveParseDelegate). Config loading and dispatch registration belong in RegisterFieldsCustom instead.</summary>");
    ...
    private static void GenerateRegisterFieldsMethod(StringBuilder sb, ProtocolInfo info)
    {
        ...
        if (info.Fields.Length > 0)
        {
            ...
            foreach (FieldInfo field in info.Fields)
            {
                if (field.IndexGroup is not null)
                {
                    ...
                    sb.AppendLine($"        {field.FieldName} = builder.RegisterFieldInGroup(protocolId, \"{EscapeString(field.Name)}\", \"{EscapeString(field.UiName)}\", {field.FieldType}, {groupConst}{desc});");
                }
                else
                {
                    string desc = field.Description is not null ? $", \"{EscapeString(field.Description)}\"" : "";
                    sb.AppendLine($"        {field.FieldName} = builder.RegisterField(protocolId, \"{EscapeString(field.Name)}\", \"{EscapeString(field.UiName)}\", {field.FieldType}{desc});");
                }
            }
            sb.AppendLine();
        }
        ...
        if (info.ProtocolTables.Length > 0)
        {
            ...
            foreach (ProtocolTableInfo table in info.ProtocolTables)
            {
                ...
                sb.AppendLine($"        {table.FieldName} = builder.RegisterProtocolTable({tableConst}, \"{EscapeString(table.UiName)}\", {_GloTableKeyType}.{table.KeyType}{desc});");
            }
            sb.AppendLine();
        }
        ...
        if (info.Settings.Length > 0)
        {
            ...
            foreach (SettingInfo setting in info.Settings)
            {
                ...
                {
                    case "Bool":
                        sb.AppendLine($"        settings.RegisterBoolSetting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {setting.DefaultValue}{desc});");
                        break;
                    case "String":
                        sb.AppendLine($"        settings.RegisterStringSetting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", \"{EscapeString(setting.DefaultValue)}\"{desc});");
                        break;
                    case "F64":
                        sb.AppendLine($"        settings.RegisterF64Setting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {setting.DefaultValue}{minMax}{desc});");
                        break;
                    case "U64":
                        sb.AppendLine($"        settings.RegisterU64Setting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {setting.DefaultValue}{minMax}{desc});");
                        break;
                    case "I64":
                        sb.AppendLine($"        settings.RegisterI64Setting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {setting.DefaultValue}{minMax}{desc});");
                        break;
                    case "Bytes":
                        {
                            ...
                            sb.AppendLine($"        settings.RegisterBytesSetting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {bytesDefault}{desc});");
                            break;
                        }
                    case "Enum":
                        {
                            ...
                            sb.AppendLine($"        settings.RegisterEnumSetting(\"{EscapeString(setting.Name)}\", \"{EscapeString(setting.UiName)}\", \"{EscapeString(setting.GroupName)}\", {setting.DefaultValue}, {_GloEnumMetadata}.FromPairs(new (string, ulong)[] {{ {enumPairs} }}){desc});");
                            break;
                        }
                }
            }
            ...
            foreach (TableRegistration reg in info.TableRegistrations)
            {
                switch (reg.KeyType)
                {
                    case "U64":
                        ...
                        break;
                    case "String":
                        sb.AppendLine($"        builder.RegisterParserInStringTableByName(\"{EscapeString(reg.Table)}\", \"{EscapeString(reg.StringKey!)}\", protocolId);");
                        break;
                    case "Bool":
                        sb.AppendLine($"        builder.RegisterParserInBoolTableByName(\"{EscapeString(reg.Table)}\", {reg.BoolKey.ToString().ToLowerInvariant()}, protocolId);");
                        break;
                    case "Bytes":
                        // Emit a literal byte array rather than a runtime Convert call for reliability and clarity.
                        sb.AppendLine($"        builder.RegisterParserInBytesTableByName(\"{EscapeString(reg.Table)}\", new {_GloBytesKey}({HexToByteArrayLiteral(reg.BytesKey!)}), protocolId);");
                        break;
                    case "Any":
    ...
    private static void EmitSettingLoadCode(StringBuilder sb, ProtocolInfo info)
    {
        ...
        foreach (SettingInfo setting in info.Settings)
        {
            ...
            {
                case "Bool":
                    sb.AppendLine($"        {setting.FieldName} = builder.Settings.GetBoolSetting(\"{EscapeString(setting.Name)}\") ?? {setting.DefaultValue};");
                    break;
                case "String":
                    sb.AppendLine($"        {setting.FieldName} = builder.Settings.GetStringSetting(\"{EscapeString(setting.Name)}\") ?? \"{EscapeString(setting.DefaultValue)}\";");
                    break;
                case "F64":
                ...
                case "Bytes":
                    {
                        string bytesDefault = string.IsNullOrEmpty(setting.DefaultValue)
                            ? "global::System.Array.Empty<byte>()"
                            : HexToByteArrayLiteral(setting.DefaultValue);
                        sb.AppendLine($"        {setting.FieldName} = builder.Settings.GetSetting(\"{EscapeString(setting.Name)}\")?.AsBytes() ?? {bytesDefault};");
                        break;
                    }
                case "Enum":
                    sb.AppendLine($"        {setting.FieldName} = builder.Settings.GetU64Setting(\"{EscapeString(setting.Name)}\") ?? {setting.DefaultValue};");
    ...
    {
        sb.AppendLine("    /// <summary>Calls the optional OnStartCustom hook. Setting values are already loaded into backing fields by RegisterFields.</summary>");
