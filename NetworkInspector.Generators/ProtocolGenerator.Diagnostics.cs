...
public sealed partial class ProtocolGenerator
{
    ...
    private static readonly DiagnosticDescriptor _DiagUnknownFieldAttribute = new(
        ...
        messageFormat: "The attribute '{0}' on field '{1}' in protocol '{2}' ends with 'FieldAttribute' but maps to no known FieldType. The field will be registered with FieldType.None.",
    ...
    private static readonly DiagnosticDescriptor _DiagInvalidBytesDefaultHex = new(
        ...
        messageFormat: "The DefaultHex value '{0}' on bytes setting '{1}' in protocol '{2}' is not a valid uppercase hexadecimal string. DefaultHex must be an even number of hex digits (e.g. \"0102AABB\").",
        category: _DiagCategory, defaultSeverity: DiagnosticSeverity.Error, isEnabledByDefault: true);
    ...
    private static readonly DiagnosticDescriptor _DiagNotPartial = new(
        ...
        messageFormat: "Protocol class '{0}' is not declared 'partial'. The source generator emits a partial class to add registration members; the user-authored declaration must therefore use the 'partial' modifier.",
        category: _DiagCategory, defaultSeverity: DiagnosticSeverity.Error, isEnabledByDefault: true);
    ...
    private static readonly DiagnosticDescriptor _DiagAttributePayloadIncomplete = new(
        ...
        messageFormat: "The '{0}' attribute on '{1}' in protocol '{2}' is missing required positional arguments and was skipped. Provide all positional constructor arguments (name, UI name, [group, ...]).",
        category: _DiagCategory, defaultSeverity: DiagnosticSeverity.Warning, isEnabledByDefault: true);
