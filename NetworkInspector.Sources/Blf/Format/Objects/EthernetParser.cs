...
internal static class EthernetParser
{
    ...
    /// </summary>
    private const int MaxEthernetPayload = 64 * 1024; // 64 KiB
    ...
    internal static bool TryParseType71(ReadOnlySpan<byte> payload, out byte[] frame, out ushort channel)
    {
        ...
        if (payload.Length < Type71HeaderSize)
        {
            return false;
        }
        ...
        // the MaxEthernetPayload cap. The payloadLen field is untrusted: a crafted value that
        ...
        int availablePayload = Math.Max(0, payload.Length - Type71HeaderSize);
        payloadLen = Math.Min(payloadLen, Math.Min(availablePayload, MaxEthernetPayload));
        ReadOnlySpan<byte> innerPayload = payload.Slice(Type71HeaderSize, payloadLen);
        ...
        return frame.Length >= MinEthernetFrameSize;
    }
    ...
    internal static bool TryParseType120(ReadOnlySpan<byte> payload, out byte[] frame, out ushort channel)
    {
        ...
        if (payload.Length < Type120HeaderSize)
        {
            return false;
        }

        channel = BinaryPrimitives.ReadUInt16LittleEndian(payload[Type120ChannelOffset..]);
        int frameLength = BinaryPrimitives.ReadUInt16LittleEndian(payload[20..]);
        int available = payload.Length - Type120HeaderSize;
        ...
        if (actualLen < MinEthernetFrameSize)
        {
            return false;
        }

        frame = payload.Slice(Type120HeaderSize, actualLen).ToArray();
        return true;
    }
    ...
    internal static bool TryParseType102(ReadOnlySpan<byte> payload, out byte[] frame, out ushort channel)
    {
        ...
        if (payload.Length < Type102HeaderSize)
        {
            return false;
        }

        channel = BinaryPrimitives.ReadUInt16LittleEndian(payload[Type102ChannelOffset..]);
        int frameLength = BinaryPrimitives.ReadUInt16LittleEndian(payload[Type102FrameLengthOffset..]);
        int available = payload.Length - Type102HeaderSize;
        ...
        frame = payload.Slice(Type102HeaderSize, actualLen).ToArray();
        return true;
    }
