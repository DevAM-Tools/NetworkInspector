...
public struct Xoroshiro128PlusPlus
{
    ...
    private const int ScramblerRotation = 17; // rotl(s0 + s1, 17)
    private const int StateRotationA = 49; // rotl(s0, 49)
    private const int StateShift = 21; // s1 << 21
    private const int StateRotationB = 28; // rotl(s1, 28)
    ...
    /// Uses two SplitMix64 rounds to expand the seed into two decorrelated state words.
    /// </summary>
    ...
    public ulong NextU64()
    {
        ...
        ulong result = BitOperations.RotateLeft(s0 + s1, ScramblerRotation) + s0;

        // State update
        s1 ^= s0;
        _S0 = BitOperations.RotateLeft(s0, StateRotationA) ^ s1 ^ (s1 << StateShift);
        _S1 = BitOperations.RotateLeft(s1, StateRotationB);
    ...
    {
        if (buffer.Length < BulkThreshold)
        {
            FillBytesSequential(buffer);
            return;
        }
        ...
        {
            FillBytesVector256(buffer, s0B, s1B, s0C, s1C, s0D, s1D);
        }
        else if (Vector128.IsHardwareAccelerated)
        {
            FillBytesVector128(buffer, s0B, s1B, s0C, s1C, s0D, s1D);
        }
        else
        {
            FillBytesScalar4(buffer, s0B, s1B, s0C, s1C, s0D, s1D);
        }
    }
        ...
        out ulong s0D, out ulong s1D)
    {
        ...
        s0B = SplitMix64(ref mix);
        s1B = SplitMix64(ref mix);
        s0C = SplitMix64(ref mix);
        s1C = SplitMix64(ref mix);
        s0D = SplitMix64(ref mix);
        s1D = SplitMix64(ref mix);

        // Guard against the (0, 0) fixed point. SplitMix64 makes this
    ...
    /// </summary>
    private void FillBytesVector256(
        Span<byte> buffer,
        ulong s0B, ulong s1B,
        ulong s0C, ulong s1C,
        ulong s0D, ulong s1D)
    {
        ...
        while (offset + BulkThreshold <= buffer.Length)
        {
            // xoroshiro128++ scrambler: result = rotl(s0 + s1, 17) + s0
            Vector256<ulong> sum = Vector256.Add(vs0, vs1);
            Vector256<ulong> result = Vector256.Add(RotateLeft256(sum, ScramblerRotation), vs0);
            ...
            vs0 = Vector256.Xor(
                Vector256.Xor(RotateLeft256(vs0, StateRotationA), t),
                Vector256.ShiftLeft(t, StateShift));
            vs1 = RotateLeft256(t, StateRotationB);

            // Write 32 bytes: element 0 (stream A) at lowest address.
            Unsafe.WriteUnaligned(ref Unsafe.Add(ref bufRef, offset), result);
            offset += BulkThreshold;
        }
        ...
        FillBytesSequential(buffer[offset..]);
    }
    ...
    /// </summary>
    private void FillBytesVector128(
        Span<byte> buffer,
        ulong s0B, ulong s1B,
        ulong s0C, ulong s1C,
        ulong s0D, ulong s1D)
    {
        ...
        while (offset + BulkThreshold <= buffer.Length)
        {
            ...
            Vector128<ulong> resultLo = Vector128.Add(RotateLeft128(sumLo, ScramblerRotation), vs0Lo);

            Vector128<ulong> tLo = Vector128.Xor(vs1Lo, vs0Lo);
            vs0Lo = Vector128.Xor(
                Vector128.Xor(RotateLeft128(vs0Lo, StateRotationA), tLo),
                Vector128.ShiftLeft(tLo, StateShift));
            vs1Lo = RotateLeft128(tLo, StateRotationB);

            // ── Streams C, D: scrambler + state update ──
            Vector128<ulong> sumHi = Vector128.Add(vs0Hi, vs1Hi);
            Vector128<ulong> resultHi = Vector128.Add(RotateLeft128(sumHi, ScramblerRotation), vs0Hi);

            Vector128<ulong> tHi = Vector128.Xor(vs1Hi, vs0Hi);
            vs0Hi = Vector128.Xor(
                Vector128.Xor(RotateLeft128(vs0Hi, StateRotationA), tHi),
                Vector128.ShiftLeft(tHi, StateShift));
            vs1Hi = RotateLeft128(tHi, StateRotationB);
        ...
        ulong s0D, ulong s1D)
    {
        ...
        while (offset + BulkThreshold <= buffer.Length)
        {
            ...
            ulong rA = BitOperations.RotateLeft(s0A + s1A, ScramblerRotation) + s0A;
            ulong rB = BitOperations.RotateLeft(s0B + s1B, ScramblerRotation) + s0B;
            ulong rC = BitOperations.RotateLeft(s0C + s1C, ScramblerRotation) + s0C;
            ulong rD = BitOperations.RotateLeft(s0D + s1D, ScramblerRotation) + s0D;

            // ── State update: 4 independent chains (no inter-dependency) ─
            ulong tA = s1A ^ s0A;
            s0A = BitOperations.RotateLeft(s0A, StateRotationA) ^ tA ^ (tA << StateShift);
            s1A = BitOperations.RotateLeft(tA, StateRotationB);

            ulong tB = s1B ^ s0B;
            s0B = BitOperations.RotateLeft(s0B, StateRotationA) ^ tB ^ (tB << StateShift);
            s1B = BitOperations.RotateLeft(tB, StateRotationB);

            ulong tC = s1C ^ s0C;
            s0C = BitOperations.RotateLeft(s0C, StateRotationA) ^ tC ^ (tC << StateShift);
            s1C = BitOperations.RotateLeft(tC, StateRotationB);

            ulong tD = s1D ^ s0D;
            s0D = BitOperations.RotateLeft(s0D, StateRotationA) ^ tD ^ (tD << StateShift);
            s1D = BitOperations.RotateLeft(tD, StateRotationB);
    ...
    /// Used for buffers shorter than <see cref="BulkThreshold"/> and for the 0–31
    /// byte remainder after the 4-stream bulk loop.
    /// </summary>
    private void FillBytesSequential(Span<byte> buffer)
    {
        if (buffer.Length == 0)
        {
            return;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector256<ulong> RotateLeft256(Vector256<ulong> v, int k) =>
        Vector256.BitwiseOr(
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector128<ulong> RotateLeft128(Vector128<ulong> v, int k) =>
        Vector128.BitwiseOr(
    ...
    /// <summary>
    /// SplitMix64: advances <paramref name="state"/> by ⌊2^64/φ⌋ and applies the
