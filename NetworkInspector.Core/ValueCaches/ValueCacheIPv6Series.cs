...
/// <summary>
/// SoA series for <see cref="FieldType.IPv6Address"/> payloads stored as high/low <see cref="ulong"/> columns.
/// <para>
/// <b>Thread-safety:</b> Single-writer / multi-reader. See <see cref="ValueCacheSeries"/>.
/// </para>
/// </summary>
public sealed class ValueCacheIPv6Series : ValueCacheSeries
{
    #region Constants
    ...
    internal ValueCacheIPv6Series(ValueCacheCapacity capacity, FieldId fieldId, ValueCaptureMode captureMode)
    {
        FieldId = fieldId;
    ...
    /// <inheritdoc/>
    public override FieldType FieldType => FieldType.IPv6Address;
    ...
    /// <summary>
    /// Gathered IPv6 row at <paramref name="index"/>.
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="index"/> is negative or not committed.</exception>
    public (int PacketId, long TimestampNanos, IPv6Address Value) this[int index]
    {
        get
        {
            ...
            return (_State.GetPacketId(index), _State.GetTimestamp(index), new IPv6Address(_High.Get(index), _Low.Get(index)));
        }
    }
