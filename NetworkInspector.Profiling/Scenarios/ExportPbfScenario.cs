...
/// <summary>
/// Profiling scenario that exports pre-parsed IPv6/UDP packets in standard PBF format
...
/// standard (row-oriented) layout and LZ4 compression.
...
internal sealed class ExportPbfScenario : ExportScenarioBase<Packet>
{
    ...
    /// <inheritdoc/>
    public override string Name => "export-pbf";

    /// <inheritdoc/>
    public override string Description =>
        FormattableString.Invariant(
            $"Export {_Batch:N0} parsed IPv6/UDP packets as PBF (Standard) → MemoryStream per iteration.");
    ...
    protected override void Export(MemoryStream stream, Packet[] packets)
    {
        using Exporters.Pbf.PbfExporter exporter = Exporters.Pbf.PbfExporter
            .CreateBuilder()
            .ToStream(stream)
            .WithFormat(Exporters.Pbf.PbfExportFormat.Standard)
            .WithCompressed(true)
            .Build();
