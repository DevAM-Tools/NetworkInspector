...
public sealed partial class LinProtocol : IProtocol
{
    ...
    private const byte PayloadLengthMask = 0xF0;

    /// <summary>Mask for the message-type field in byte 4 (bits 3-2).</summary>
    private const byte MsgTypeMask = 0x0C;

    /// <summary>Mask for the checksum-type field in byte 4 (bits 1-0).</summary>
    private const byte ChecksumTypeMask = 0x03;

    /// <summary>Message type: standard frame (unconditional or sporadic).</summary>
    private const byte MsgTypeFrame = 0;

    /// <summary>Message type: event-triggered frame.</summary>
    private const byte MsgTypeEvent = 3;

    /// <summary>Mask for the 6-bit frame ID within the PID byte.</summary>
    private const byte FrameIdMask = 0x3F;

    // Error flag bits in byte 7 (per Wireshark packet-lin.h)
    private const byte ErrNoSlaveResponse = 0x01;
    private const byte ErrFraming = 0x02;
    private const byte ErrParity = 0x04;
    private const byte ErrChecksum = 0x08;
    private const byte ErrInvalidId = 0x10;
    private const byte ErrOverflow = 0x20;

    /// <summary>Index group for always-present LIN fields.</summary>
    private const string LinIndexGroup = "lin";
    ...
    [BytesField("lin", "LIN", IndexGroup = LinIndexGroup)]
    ...
    public ParseResult Parse(in MutField parentField, ReadOnlyMemory<byte> data, in ParseContext context)
    {
        ...
        // ── Byte 4: Payload-Length[7:4] | Msg-Type[3:2] | Checksum-Type[1:0] ─
        byte byte4 = span[4];
        byte payloadLength = (byte)((byte4 & PayloadLengthMask) >> 4);
        byte msgType = (byte)((byte4 & MsgTypeMask) >> 2);
        byte checksumType = (byte)(byte4 & ChecksumTypeMask);

        // ── Byte 5: PID (Parity[7:6] | Frame-ID[5:0]) ────────────────────────
        byte pid = span[5];
        byte frameId = (byte)(pid & FrameIdMask);
        ...
        int actualDataLen = Math.Min((int)payloadLength, data.Length - HeaderSize);
        int totalConsumed = HeaderSize + actualDataLen;
        ...
        string msgTypeSummaryText = msgType == MsgTypeEvent ? "Event" : "Frame";
    ...
    /// </summary>
    private static bool ValidateChecksum(ReadOnlySpan<byte> dataBytes, byte pid, byte checksumType, byte expected)
    {
    ...
    /// </summary>
    private static bool ValidateParity(byte frameId, byte parity)
    {
        int id0 = (frameId >> 0) & 1;
        int id1 = (frameId >> 1) & 1;
        int id2 = (frameId >> 2) & 1;
