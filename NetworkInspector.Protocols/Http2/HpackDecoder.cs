...
/// <summary>
/// Lightweight HPACK header compression decoder (RFC 7541).
/// Decodes HPACK-encoded header blocks into name/value pairs.
/// This is a per-packet stateless decoder (does not maintain a dynamic table across packets).
/// </summary>
internal static class HpackDecoder
{
    /// <summary>Represents a decoded HPACK header (name + value).</summary>
    ...
    /// Uses the static table for indexed headers and literal decoding for others.
    /// Dynamic table entries are tracked within this single header block only.
    /// </summary>
    /// <param name="data">The HPACK-encoded header block fragment.</param>
    /// <returns>List of decoded headers, or empty list if decoding fails.</returns>
    internal static List<Header> Decode(ReadOnlySpan<byte> data)
    {
        List<Header> headers = [];

        // Per-block dynamic table to track headers added within this block
        List<Header> dynamicTable = [];
        ...
        while (offset < data.Length)
        {
            ...
            else if ((b & 0xC0) == 0x40)
            {
                ...
                dynamicTable.Insert(0, header.Value); // Prepend to dynamic table
                headers.Add(header.Value);
            }
            else if ((b & 0xF0) == 0x00)
            {
    ...
    private static int DecodeInteger(ReadOnlySpan<byte> data, ref int offset, int prefixBits)
    {
        ...
        while (offset < data.Length)
        {
            ...
            // Guard against excessively large integers
            if (shift > 28)
            {
                return -1;
            }
    ...
    private static string? HuffmanDecode(ReadOnlySpan<byte> data)
    {
        ...
        foreach (byte b in data)
        {
            ...
            // Try to extract symbols while we have enough bits
            while (bits >= 5) // Minimum Huffman code length is 5 bits
            {
                bool found = false;
                // Try to match from longest to shortest code
                for (int len = Math.Min(bits, 30); len >= 5; len--)
                {
    ...
    #region HPACK Static Table (RFC 7541, Appendix A)
    ...
    [
        ...
        new(":status", "404"),          // 13
        new(":status", "500"),          // 14
        new("accept-charset", ""),      // 15
        new("accept-encoding", "gzip, deflate"), // 16
        new("accept-language", ""),     // 17
        new("accept-ranges", ""),       // 18
        new("accept", ""),              // 19
        new("access-control-allow-origin", ""), // 20
        new("age", ""),                 // 21
        new("allow", ""),               // 22
        new("authorization", ""),       // 23
        new("cache-control", ""),       // 24
        new("content-disposition", ""), // 25
        new("content-encoding", ""),    // 26
        new("content-language", ""),    // 27
        new("content-length", ""),      // 28
        new("content-location", ""),    // 29
        new("content-range", ""),       // 30
        new("content-type", ""),        // 31
        new("cookie", ""),              // 32
        new("date", ""),                // 33
        new("etag", ""),                // 34
        ...
        new("host", ""),                // 38
        new("if-match", ""),            // 39
        new("if-modified-since", ""),   // 40
        new("if-none-match", ""),       // 41
        new("if-range", ""),            // 42
        new("if-unmodified-since", ""), // 43
        new("last-modified", ""),       // 44
        new("link", ""),                // 45
        new("location", ""),            // 46
        new("max-forwards", ""),        // 47
        new("proxy-authenticate", ""),  // 48
        new("proxy-authorization", ""), // 49
        new("range", ""),               // 50
        new("referer", ""),             // 51
        new("refresh", ""),             // 52
        new("retry-after", ""),         // 53
        new("server", ""),              // 54
        new("set-cookie", ""),          // 55
        new("strict-transport-security", ""), // 56
        new("transfer-encoding", ""),   // 57
        new("user-agent", ""),          // 58
        new("vary", ""),                // 59
        new("via", ""),                 // 60
        new("www-authenticate", ""),    // 61
    ];

    #endregion

    #region HPACK Huffman Code Table (RFC 7541, Appendix B)
