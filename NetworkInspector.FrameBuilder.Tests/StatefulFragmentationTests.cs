...
internal sealed class StatefulFragmentationTests
{
    ...
    private static ushort ReadIPv4FlagsField(ReadOnlySpan<byte> frame)
        => BinaryPrimitives.ReadUInt16BigEndian(frame.Slice(EthHeaderSize + 6, 2));
    ...
    public async Task StatefulIPv4_OversizePayload_EmitsMultipleFragments_WithSameIPID()
    {
        ...
        await Assert.That(ReadIPv4Id(fragments[0])).IsEqualTo<ushort>(4242);
        await Assert.That(ReadIPv4Id(fragments[1])).IsEqualTo<ushort>(4242);
        await Assert.That(ReadIPv4Id(fragments[2])).IsEqualTo<ushort>(4242);

        // Fragment 0: MF=1, offset=0; Fragment 1: MF=1, offset=183; Fragment 2: MF=0, offset=366.
        ushort f0 = ReadIPv4FlagsField(fragments[0]);
        ushort f1 = ReadIPv4FlagsField(fragments[1]);
        ushort f2 = ReadIPv4FlagsField(fragments[2]);
    ...
    public async Task StatefulIPv4_OversizeAcrossTwoPackets_AdvancesIPIdByOnePerPacket()
    {
        ...
        {
            (ushort id, int count) = DrainPacket(session, payload);
            if (packet == 0)
            {
                firstId = id;
    ...
    private static (ushort firstFragmentId, int fragmentCount) DrainPacket(
        ...
        ReadOnlySpan<byte> payload)
    {
        ...
        while (seq.MoveNext(scratch, out int n))
        {
            if (fragIdx == 0)
            {
                firstFragmentId = ReadIPv4Id(scratch);
            }
            fragIdx++;
        }
    ...
    public async Task StatefulTcp_OversizePayload_TcpSeqAdvancesByFullPayloadLength()
    {
        ...
        uint seqP1 = DrainTcpPacketFirstSeq(session, payload);

        // Packet 2.
        uint seqP2 = DrainTcpPacketFirstSeq(session, payload);
    ...
    private static uint DrainTcpPacketFirstSeq(
    ...
    private struct ReentrantFragmentInterceptor : FB.IFrameInterceptor
    {
        ...
        public void OnFrameComplete(scoped Span<byte> frame)
        {
            ...
            FB.UdpLayer udp = new(7777, 8888, FB.Auto<ushort>.Explicit(0));
    ...
    public async Task Scratch_ReentrantBuildFromInterceptor_DoesNotCorruptOuterFragments()
    {
        ...
        {
            byte[] srcIpBytes = outerFragments[i].AsSpan(EthHeaderSize + 12, 4).ToArray();
            byte[] dstIpBytes = outerFragments[i].AsSpan(EthHeaderSize + 16, 4).ToArray();
