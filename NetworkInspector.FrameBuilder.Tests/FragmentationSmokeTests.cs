...
internal sealed class FragmentationSmokeTests
{
    ...
    private const int IPv6FragHeaderSize = 8;
    ...
    public async Task IPv4_LargePayload_EmitsMultipleFragments_WithMfAndOffsetChain()
    {
        ...
        List<byte[]> fragments = CollectFragments(in stack, payload, MtuFrameBytes);

        await Assert.That(fragments.Count).IsEqualTo(3);

        // Per-fragment slice geometry: 1464, 1464, 80 (= 3008 - 2*1464).
        await Assert.That(fragments[0].Length).IsEqualTo(EthHeaderSize + IPv4HeaderSize + 1464);
        await Assert.That(fragments[1].Length).IsEqualTo(EthHeaderSize + IPv4HeaderSize + 1464);
        await Assert.That(fragments[2].Length).IsEqualTo(EthHeaderSize + IPv4HeaderSize + 80);

        // Fragment 0: MF=1, FragOffset=0.
        ushort flags0 = BinaryPrimitives.ReadUInt16BigEndian(fragments[0].AsSpan(EthHeaderSize + 6, 2));
        ...
        ushort flags1 = BinaryPrimitives.ReadUInt16BigEndian(fragments[1].AsSpan(EthHeaderSize + 6, 2));
        ...
        ushort flags2 = BinaryPrimitives.ReadUInt16BigEndian(fragments[2].AsSpan(EthHeaderSize + 6, 2));
        ...
        ushort id0 = BinaryPrimitives.ReadUInt16BigEndian(fragments[0].AsSpan(EthHeaderSize + 4, 2));
        ushort id1 = BinaryPrimitives.ReadUInt16BigEndian(fragments[1].AsSpan(EthHeaderSize + 4, 2));
        ushort id2 = BinaryPrimitives.ReadUInt16BigEndian(fragments[2].AsSpan(EthHeaderSize + 4, 2));
        ...
        ushort total0 = BinaryPrimitives.ReadUInt16BigEndian(fragments[0].AsSpan(EthHeaderSize + 2, 2));
        ushort total2 = BinaryPrimitives.ReadUInt16BigEndian(fragments[2].AsSpan(EthHeaderSize + 2, 2));
        await Assert.That(total0).IsEqualTo((ushort)(IPv4HeaderSize + 1464));
        await Assert.That(total2).IsEqualTo((ushort)(IPv4HeaderSize + 80));
        ...
        {
            await Assert.That(VerifyOnesComplementZero(fragments[i].AsSpan(EthHeaderSize, IPv4HeaderSize))).IsTrue();
        }
    }
    ...
    public async Task IPv4_DontFragment_True_ProducesFragmentationRequiredStatus()
    {
        ...
        byte[] dst = new byte[MtuFrameBytes];
        bool wrote;
        int written;
    ...
    public async Task IPv4_SubMtuPayload_StillEmitsExactlyOneFrame()
    {
        ...
        await Assert.That(fragments[0].Length).IsEqualTo(EthHeaderSize + IPv4HeaderSize + UdpHeaderSize + payload.Length);
        ...
        ushort flags = BinaryPrimitives.ReadUInt16BigEndian(fragments[0].AsSpan(EthHeaderSize + 6, 2));
    ...
    public async Task IPv6Fragment_LargePayload_EmitsMultipleFragments_WithMfAndOffsetChain()
    {
        ...
        await Assert.That(fragments[0][EthHeaderSize + 6]).IsEqualTo(IpProtocols.IPv6Fragment);
        // Frag header: NextHeader = UDP (17).
        const int FragOffsetInFrame = EthHeaderSize + IPv6HeaderSize;
        ...
        ushort plen0 = BinaryPrimitives.ReadUInt16BigEndian(fragments[0].AsSpan(EthHeaderSize + 4, 2));
        await Assert.That(plen0).IsEqualTo((ushort)(IPv6FragHeaderSize + 1432));
    }
    ...
    private static List<byte[]> CollectFragments<TStack, TTrailer, TInterceptor>(
    ...
    /// </summary>
    private static bool VerifyOnesComplementZero(ReadOnlySpan<byte> header)
    {
        uint sum = 0;
